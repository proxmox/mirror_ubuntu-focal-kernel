...
static __attribute__ ((aligned (PAGE_SIZE))) uint8_t pages[2][PAGE_SIZE];
...
static void guest_code(void)
{
	...
	GUEST_ASSERT(set_storage_key(page_store_prot, 0x10) == 0);
	GUEST_ASSERT(set_storage_key(page_fetch_prot, 0x98) == 0);
...
int main(int argc, char *argv[])
{
	...
	guest_0_page = vm_vaddr_alloc(vm, PAGE_SIZE, 0, 0, 0);
	if (guest_0_page != 0)
		fprintf(stderr,
			"Did not allocate page at 0 for fetch protection override tests, skipping test\n");
